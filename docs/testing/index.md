...
CIは `main` / `develop` へのpushと、両ブランチを対象とするpull requestで実行します。`verification_scope` が `scripts/classify_verification_inputs.py` の次の10 booleanを出力します。

`backend`, `frontend`, `backend_container`, `deploy_preflight`, `governance`, `workflow_contract`, `dependency_review`, `playwright_smoke`, `playwright_visual`, `classification_ok`
