...
## CI 実行
- PR 向けのスモーク（`pull_request`）: `auth.spec.ts` / `guest.spec.ts` / `wordpack-active-conditions.spec.ts` / `wordpack-list-states.spec.ts` / `wordpack-server-query.spec.ts` / `wordpack.spec.ts` の主要導線と、一覧の適用中条件、空・該当なし・失敗・再読み込み状態、201件境界をまたぐ全ページ検索・ページリセットを実行します。
