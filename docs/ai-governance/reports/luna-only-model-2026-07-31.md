...
| P2 | 全生成ジョブ | 同じUUIDを同一所有者・種別の別入力へ再利用できる | 新しい入力を実行せず、古い成功結果を新しい対象の結果として表示し得る | 操作対象と生成入力のcanonical fingerprintを保存・照合し、不一致を409で拒否 | 対応済 |
...
| Cloud Run インスタンス強制終了時のジョブ自動再実行 | Cloud Tasks 等の外部キュー導入は今回のモデル移行範囲を超える | 実行中ジョブが `running` のまま残る可能性 | 理由、ユーザー影響、冪等性・復旧条件を明記した後続 Issue #565 で耐久キュー化する |
