...
- `before` は base commit `21dfbe355eec5d48d1ffc89fc1df826e95f2ace4` の clean temporary worktreeで取得した。現行 candidate は `HEAD` `cb5e0c0f72e050dff67f7f1d179665302ac41280` に PR #627 の最終 P2 修正を加えた状態である。
- base から現行 candidate までの実装対象5 pathの snapshot fingerprint は `5b0f12c017453bb0cafb4ff2cb8004c7180303e86849d39a46671a4dca448af8`、新規 `WordPackTransientMessage.tsx` の SHA-256 は `9ea7ebb65771aba9d164cc0075a88906e8dd8bd36d6b6534f6b1d288cb47b1d4`。現行未コミット P2 working diff（2 path）の fingerprint は `28d17f7a17142ff0d0ae634c8e52994306b8b058538fd9322beebae070231436` である。
...
- 今回の追従で書き込んだのは本 report のみ。commit、push、実装 path の編集はしていない。
...
| P1 | Introduced | Clipboard の完了順、WordPack 切替、unmount 後の古い結果が現在の preview に表示されるリスク。operation id、WordPack identity context、mounted guard を現行実装で確認し、逆順完了の回帰 test が pass した（`useExampleActions.ts:401-458`、`WordPackPanel.test.tsx:546-636`）。 | 解消を確認。 |
| P2 | Regression (PR #627) | Clipboard pending 中に同じ WordPack の mutable data（`ゲスト公開` 更新）が変わると、hook が settlement を破棄して完了通知を出さないリスク。settlement 判定を `currentWordPackId` identity に限定し、pending → guest-public update → resolve の回帰 test が pass した（`useExampleActions.ts:64-79,401-458`、`WordPackPanel.test.tsx:425-488`）。 | 解消を確認。 |
...
今回の追加修正は P2 のみに限定され、追加の包括 review は実施していない。上記の mutable data に関する P2 は、現行 candidate の回帰 test と source review により解消扱いとした。
...
- 既存の full coverage result（48 test files passed、1 skipped; 260 passed、1 skipped）は、この source / test 変更により失効した。今回の lane では full suite を再実行せず、最終 gate での再取得待ちである。
- `git diff --check`: **PASS**。
...
- 既存の生成通知経路は copy action から利用せず、生成・import 側の notification 契約を変更していないことを差分と focused regression test で確認した。現行 candidate の full suite による再確認は最終 gate 待ちである。
...
- PR #627 の current diff は transient message の CSS と `WordPackTransientMessage` の markup を変更せず、Clipboard settlement の `currentWordPackId` identity 判定と回帰 test のみを更新していることを source / diff で確認した。したがって、以下の after 画像は表示内容について現行 candidateへ再利用可能と判断し、画像自体は再取得・変更していない。

## 画面証跡の provenance

取得時刻は Asia/Tokyo、画像は接続済み browser の `fullPage: false` screenshot API で取得した JPEG である。取得後に dimensions、形式、表示内容を確認した。`after` 画像は PR #627 の最終 P2 修正前に取得した visual candidate由来だが、現行 candidateの P2 diff は可視 CSS / component markup を変更していないことを source / diff で確認したため、その表示証跡を再利用している。
...
| [after: desktop success](../evidence/issue-550/lexicon-copy-after-final.jpg) | pre-PR #627 final P2 visual candidate; current candidate reuse validated (`5b0f12…`, component `9ea7ebb…`) | 2026-08-29 19:08:30 +09:00 | 1280×900 / 1280×900 | Dev 例文を Copy、250ms 後に撮影。上部 success message、queue 0件。 |
| [after: mobile success](../evidence/issue-550/lexicon-copy-after-mobile.jpg) | pre-PR #627 final P2 visual candidate; current candidate reuse validated (`5b0f12…`, component `9ea7ebb…`) | 2026-08-29 19:09:55 +09:00 | 390×844 / 375×812 | モーダル内を Dev 例文まで scroll、Copy、250ms 後に撮影。2行表示、横 overflow なし。 |
...
```text
 UserManual.md                                      |   1 +
 apps/frontend/src/WordPackPanel.test.tsx           | 317 ++++++++++++++++++++-
 .../WordPackPanel/WordPackPanelContainer.tsx       |  71 +++++
 .../WordPackPanel/WordPackTransientMessage.tsx     |  45 +++
 apps/frontend/src/hooks/useExampleActions.ts       |  64 +++++-
 5 files changed, 488 insertions(+), 10 deletions(-)
```

上記は base `21dfbe3…` から現行 working treeまでの実装対象5 pathの stat である。現行未コミット PR #627 working diff（2 path）は次のとおりである。

```text
 apps/frontend/src/WordPackPanel.test.tsx     | 21 +++++++++++++++++----
 apps/frontend/src/hooks/useExampleActions.ts | 10 ++++------
