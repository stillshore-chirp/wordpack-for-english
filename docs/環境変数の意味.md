...
- 既定値:
  - env.example: `LLM_PROVIDER=openai`, `LLM_MODEL=gpt-5.6-luna`, `LLM_TIMEOUT_MS=300000`, `LLM_MAX_RETRIES=1`, `LLM_MAX_TOKENS=25000`
  - コード既定: `llm_provider="openai"`, `llm_model="gpt-5.6-luna"`, `llm_timeout_ms=300000`, `llm_max_retries=1`, `llm_max_tokens=25000`
- 使われ方:
  - プロバイダ選択と初期化
...
- 未設定/誤設定:
  ...
  - `LLM_TIMEOUT_MS=300000` は Luna High の推論を含む1試行に5分を与える。アプリ内のHTTP全体上限は +5秒。Cloud Run はこれより長い6分以上を設定し、Firebase Hosting リライト経由の約60秒制約がある同期導線では直接接続または非同期ジョブ・ポーリングを使う。
