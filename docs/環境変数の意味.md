...
### 2) LLM_PROVIDER, LLM_MODEL, REQUEST_TIMEOUT_MS, LLM_TIMEOUT_MS, LLM_REQUEST_TIMEOUT_MS, LLM_MAX_RETRIES, LLM_MAX_TOKENS
- 用途:
  - `LLM_PROVIDER`: LLM クライアントの選択。`openai` | `local`
- `LLM_MODEL`: 使うモデル名（現在は `gpt-5.6-luna` のみ）
  - `LLM_TIMEOUT_MS`: 1回の LLM 呼び出しのタイムアウト
  - `LLM_REQUEST_TIMEOUT_MS`: 複数の LLM 呼び出しを含む HTTP リクエスト全体のタイムアウト
  - `REQUEST_TIMEOUT_MS`: 一覧・詳細・更新・ジョブ状態確認など通常APIのタイムアウト
  - `LLM_MAX_RETRIES`: LLM 呼び出しの最大リトライ回数
- 既定値:
  - env.example: `LLM_PROVIDER=openai`, `LLM_MODEL=gpt-5.6-luna`, `REQUEST_TIMEOUT_MS=60000`, `LLM_TIMEOUT_MS=300000`, `LLM_REQUEST_TIMEOUT_MS=1500000`, `LLM_MAX_RETRIES=1`, `LLM_MAX_TOKENS=25000`
  - コード既定: `llm_provider="openai"`, `llm_model="gpt-5.6-luna"`, `request_timeout_ms=60000`, `llm_timeout_ms=300000`, `llm_request_timeout_ms=1500000`, `llm_max_retries=1`, `llm_max_tokens=25000`
- 使われ方:
  - プロバイダ選択と初期化
...
- 未設定/誤設定:
  ...
  - `REQUEST_TIMEOUT_MS=60000` は通常APIに1分を与える。`LLM_TIMEOUT_MS=300000` は Luna High の推論を含む1試行に5分、`LLM_REQUEST_TIMEOUT_MS=1500000` は最大4回の直列LLM呼び出しを含む生成フロー全体に25分を与える。`/api/config` は通常値と生成値を別々に返す。同期 `/api/article/import` はイベントループをブロックし、同期 worker thread を使う `/api/article/generate_and_import` はthread内の保存処理を安全に停止できないため、両方をアプリ内ASGI timeoutの対象外とし、Cloud Runのtimeoutを最終境界にする。Reader取り込みは非同期ジョブ・ポーリングを使い、Cloud Run では `CLOUD_RUN_NO_CPU_THROTTLING=true` と30分以上のtimeoutを設定する。
  - `LLM_MAX_TOKENS` は Responses API の `max_output_tokens` として使用され、推論・可視出力・非表示の整形トークンを合わせた生成上限になる。`reasoning.effort=high` で可視出力前に上限へ達することを避けるため、OpenAI の初期検証時の推奨余裕に合わせて 25,000 を既定とする。上限値そのものではなく、実際に生成したトークンが課金対象。
- 設定例:
  - OpenAI: `LLM_PROVIDER=openai`, `LLM_MODEL=gpt-5.6-luna` + `OPENAI_API_KEY`
  - オフライン学習/テスト: `LLM_PROVIDER=local`
  - UI から `reasoning.effort`（`none|low|medium|high|xhigh|max`、既定 `high`）/`text.verbosity`（`low|medium|high`、既定 `medium`）を指定
...
### 5) LANGFUSE_LOG_FULL_PROMPT, LANGFUSE_LOG_FULL_OUTPUT

- 用途: Langfuse に raw prompt / output を記録する限定調査用 opt-in。
- 既定値: どちらも `false`。production の `env.deploy.example` も安全側の `false`。
- 上限: `LANGFUSE_PROMPT_MAX_CHARS` と `LANGFUSE_OUTPUT_MAX_CHARS` で個別に制限する。
- 未設定時: 文字数と SHA-256 hash だけを記録し、raw content は送信しない。
- 運用: 通常は `LANGFUSE_ENABLED=false` でも generation provenance と構造化ログを利用できる。詳細は [LLMOps privacy](llmops/privacy-and-operations.md) を参照。
...
- ステージング/本番（OpenAI）:
  - `LLM_PROVIDER=openai`, `LLM_MODEL=gpt-5.6-luna`, `OPENAI_API_KEY=...`
