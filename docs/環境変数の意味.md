...
- 未設定/誤設定:
  ...
  - `REQUEST_TIMEOUT_MS=60000` は通常APIに1分を与える。`LLM_TIMEOUT_MS=300000` は Luna High の推論を含む1試行に5分、`LLM_REQUEST_TIMEOUT_MS=1500000` は最大4回の直列LLM呼び出しを含む生成フロー全体に25分を与える。`/api/config` は通常値と生成値を別々に返す。同期 worker thread を使う `/api/article/generate_and_import` はアプリ内の全体timeoutで停止できないため、そのmiddleware対象外とし、Cloud Runのtimeoutを最終境界にする。Reader取り込みは非同期ジョブ・ポーリングを使い、Cloud Run では `CLOUD_RUN_NO_CPU_THROTTLING=true` と30分以上のtimeoutを設定する。
