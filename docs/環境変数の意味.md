...
### 2) LLM_PROVIDER, LLM_MODEL, LLM_TIMEOUT_MS, LLM_REQUEST_TIMEOUT_MS, LLM_MAX_RETRIES, LLM_MAX_TOKENS
- 用途:
...
- 既定値:
  - env.example: `LLM_PROVIDER=openai`, `LLM_MODEL=gpt-5.6-luna`, `LLM_TIMEOUT_MS=300000`, `LLM_REQUEST_TIMEOUT_MS=1500000`, `LLM_MAX_RETRIES=1`, `LLM_MAX_TOKENS=25000`
  - コード既定: `llm_provider="openai"`, `llm_model="gpt-5.6-luna"`, `llm_timeout_ms=300000`, `llm_request_timeout_ms=1500000`, `llm_max_retries=1`, `llm_max_tokens=25000`
- 使われ方:
  - プロバイダ選択と初期化
...
- 未設定/誤設定:
  ...
  - `LLM_TIMEOUT_MS=300000` は Luna High の推論を含む1試行に5分を与える。`LLM_REQUEST_TIMEOUT_MS=1500000` は Reader取り込みなど最大4回の直列LLM呼び出しを含むフロー全体に25分を与え、`/api/config` を通じてフロントのabortにも同期する。アプリ内HTTP middlewareは全体上限 +5秒。Cloud Run は30分以上を設定し、Firebase Hosting リライト経由の約60秒制約がある同期導線では直接接続または非同期ジョブ・ポーリングを使う。
