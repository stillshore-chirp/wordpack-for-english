...
- [x] `production` environment limits deployments to `main`; the current reviewer count is 0 and the wait timer is 0.
- [ ] Deployment secrets are scoped to the production environment where possible.
- [ ] Repository variables `GCP_PROJECT_ID`, `GCP_DEPLOY_WIF_PROVIDER`, `GCP_PREFLIGHT_WIF_PROVIDER`, `GCP_DEPLOY_SERVICE_ACCOUNT`, and `GCP_PREFLIGHT_SERVICE_ACCOUNT` are configured without putting their values in source control; this snapshot does not re-confirm their live inventory.
- [x] Repository variable `GCP_BUILD_SERVICE_ACCOUNT` is configured; its value is not recorded in this document.
- [x] Repository variable `PRODUCTION_DEPLOY_ENABLED` is currently the literal string `true`; this setting is not evidence of a successful production deploy.
- [x] `CLOUD_RUN_ENV_FILE_BASE64` is the only production deployment secret referenced by the workflows and is repository-scoped; no environment secret is currently configured.
...
- `Deploy to production` accepts automatic deployment only through the CI main-push `quality_gate` job's local `workflow_call`; the caller passes its exact `github.sha` and current `github.run_id`, and the called workflow verifies the CI workflow identity, main push, matching SHA/run, and the same run's successful `Quality gate (selected checks)` job. No automatic `workflow_run` trigger exists.
- CI authorization also pins `.github/workflows/ci.yml` to live workflow ID `187172373` and requires the same run's canonical `Quality gate (selected checks)` job to be completed successfully; a recreated workflow requires an explicit constant update. The called production jobs are part of the caller's GitHub Actions check suite.
- Manual break-glass requires an explicit `target_sha` and a dispatch from the trusted `main` ref; the workflow queries GitHub Actions for a matching completed `CI` push on `main` and then requires exactly one completed/successful canonical `Quality gate (selected checks)` job before entering the `production` environment. The overall CI run conclusion need not be success, because an automatic deploy failure can fail the enclosing run after Quality gate succeeds. The optional `identity_exchange_only=true` path still requires this target/Quality gate verification and only checks deploy WIF token exchange.
...
- `prepare-release-artifacts`, `build-backend-artifact`, `attest-backend-artifact`, and `deploy` are four jobs in the existing production workflow. Prepare checks out that exact SHA, builds the frontend, and uploads a one-day handoff artifact. Build checks out and asserts `git rev-parse HEAD`; `scripts/build_backend_artifact.sh` creates a mode-700 private context with `git archive TARGET_SHA` and submits it to Cloud Build exactly once; dirty, ignored, and untracked files cannot enter the context. The one Cloud Build input tag and runtime `GIT_SHA` are derived from that checkout and cannot be overridden by the deployment env file; Cloud Run receives only the verified full `IMAGE_URI` (`@sha256:<64-hex>`). Build derives a deterministic target-SHA local handoff tag from the verified immutable registry image, verifies the same image ID, archives that tag, and passes the archive SHA-256 to Attest. Attest verifies the checksum before `docker load`, checks the restored handoff tag, runs checksum-pinned Syft 1.51.1 against that local Docker daemon tag, and writes GitHub delivery attestations whose subject remains the immutable registry digest. Deploy downloads only the handoff artifacts and performs the final private-GAR verification and digest-only release.
- The `build-backend-artifact` and `deploy` jobs, plus authenticated preflight and the manual main identity-only job, use job-scoped Workload Identity Federation for their Cloud Build, private-GAR verification, and probe needs. The main-only `deploy_production` CI caller supplies only the called jobs' permission union (`contents: read`, `actions: read`, `id-token: write`, `attestations: write`); ordinary PR/CI jobs do not receive OIDC. `attest-backend-artifact` has only the job-scoped GitHub OIDC permission required by `actions/attest` and receives no Google credential; `verify-target`, `prepare-release-artifacts`, and `authorize-deploy-cutover` do not receive `id-token: write`.
...
- `build-backend-artifact` compares the Cloud Build result with the Artifact Registry manifest digest and obtains native provenance for that exact digest with `--show-provenance`. It verifies the image digest, GoogleHostedWorker, invocation, and `_SOURCE_REPOSITORY` / `_TARGET_SHA` / `_BUILDER_WORKFLOW` substitutions. SCM metadata may be absent for the local archive; when present, repository/ref/SHA are checked strictly. The verified JSON is SHA256-bound into `nativeProvenanceSnapshotSha256`. It also runs the health smoke against the exact digest, then derives and verifies a same-image-ID target-SHA local handoff tag, archives that tag, and passes its SHA-256 to `attest-backend-artifact`. Attest checks the archive checksum before `docker load`, verifies the restored handoff tag, and runs checksum-pinned Syft 1.51.1 against the local handoff tag to produce strict SPDX 2.3 JSON; it does not use an Anchore Action. The resulting GitHub delivery predicate (`https://github.com/stillshore-chirp/wordpack-for-english/attestations/backend-delivery/v1`) and SBOM attestations keep the immutable registry image digest as their subject. In GitHub's verification certificate, `--source-digest` is the source repository digest (`github.sha`) and `--signer-digest` is the workflow file revision (`github.workflow_sha`); the build target `TARGET_SHA` remains a separate custom delivery-predicate field. The delivery predicate's `runDetails.builder.id` is the exact production `BUILDER_WORKFLOW` URI and its payload `buildDefinition.buildType` is `${BUILDER_WORKFLOW}#backend-cloud-build-v1`; `underlyingBuilder=https://cloudbuild.googleapis.com/GoogleHostedWorker` and `cloudBuildProvenance=required` are values checked as the claimed upstream Cloud Build native provenance. The GitHub signer identity is the workflow certificate, not the Google builder identity. Cloud Build native provenance is independently required as reserved SLSA predicate `https://slsa.dev/provenance/v1` with `options.requestedVerifyOption: VERIFIED`. Google credentials are removed before Syft/actions/attest and re-authenticated in the deploy job immediately before private-GAR `gh attestation verify`. A missing or mismatched source digest, signer digest, target SHA, repository, signer workflow, builder, digest, native provenance, or SBOM fails closed before Cloud Run, traffic, or Hosting writes.
- The production workflow has no unconditional PR publish path for backend images, SBOMs, or attestations. No new workflow is introduced; the four handoff jobs are added inside the existing workflow because job-wide OIDC/permissions and build, Syft/attestation, and deployment dependencies must remain isolated. The normal exact-SHA, CI/Quality gate, candidate/canary/health/rollback, and materialized env cleanup remain required. `build-backend-artifact` and `deploy` both use the `production` environment; the current reviewer count and wait timer are 0, so these jobs require no manual approval while the main-only deployment policy remains enforced. The manual identity-only path is also environment-bound but performs only WIF exchange. For manual direct deploy tokens, `job_workflow_ref` may be absent; the deploy WIF provider maps it with an empty-string fallback. The manual direct path pins `assertion.workflow_ref` to `stillshore-chirp/wordpack-for-english/.github/workflows/deploy-production.yml@refs/heads/main`; the reusable auto path pins the caller `assertion.workflow_ref` to `stillshore-chirp/wordpack-for-english/.github/workflows/ci.yml@refs/heads/main` and the mapped `attribute.job_workflow_ref` to the exact deploy workflow. Both paths retain the repository/owner ID, main ref, and production environment restrictions.
...
- [x] GitHub `production` environment has a `main`-only deployment branch policy; the current reviewer count is 0 and the wait timer is 0.
- [x] `PRODUCTION_DEPLOY_ENABLED` is currently the literal string `true`; this does not claim that a production deploy succeeded.
- [x] The live deploy Workload Identity Provider is active and requires numeric repository/owner IDs, main ref, deploy-only `production` environment, exact manual `workflow_ref`, or the exact reusable-auto CI caller `workflow_ref` plus deploy `job_workflow_ref` documented in [`docs/deployment.md`](../deployment.md). The optional `job_workflow_ref` mapping preserves manual tokens with an empty fallback; existing mappings and non-trust provider fields were unchanged by the update. The separate preflight provider contract remains unchanged.
- [ ] `roles/iam.workloadIdentityUser` uses separate exact deploy-environment/preflight-main subjects, and the preflight service account has only the custom Firestore index-list role plus `roles/firebasehosting.viewer`.
...
- [ ] `containeranalysis.googleapis.com` is enabled and the deploy service account has project-level `roles/containeranalysis.occurrences.viewer` for exact-digest native provenance reads; concrete project and account identifiers remain outside this repository.
- [x] The dedicated build service account is configured; its `roles/logging.logWriter` and other build input/output roles still require live IAM confirmation.
- [x] `CLOUD_RUN_ENV_FILE_BASE64` is the only deployment secret referenced by the workflows, is repository-scoped, and has no configured environment-secret counterpart.
- [ ] Deployment secrets are scoped to the `production` environment where possible; the current environment-secret inventory is empty.
