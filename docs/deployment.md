...
- 手動の break-glass 実行は trusted `main` ref からのみ起動でき、必須入力 `target_sha` を受け取ります。GitHub API で同一 SHA の `CI` 成功・push・main run を確認してから `production` environment job へ進みます。
